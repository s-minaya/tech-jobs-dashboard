...
// CSS del componente integrado como <style> inline.
// Evita depender de archivos .css externos y mantiene consistencia con Tailwind.
const LIGHTFALL_STYLES = `
  .lightfall-container {
    position: relative;
    width: 100%;
    height: 100%;
...
// Lightfall
// Efecto WebGL de luces cayendo en túnel (React Bits).
// CSS integrado como <style> inline para no depender de archivos .css externos.
const Lightfall = ({
  className, dpr, paused = false,
  colors = ['#A6C8FF', '#5227FF', '#FF9FFC'],
  backgroundColor = '#0A29FF', speed = 0.5, streakCount = 8,
  streakWidth = 1, streakLength = 1, glow = 1, density = 1,
  twinkle = 1, zoom = 2, backgroundGlow = 1, opacity = 1,
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uGlow:         { value: glow },       uDensity:      { value: density },
      uTwinkle:      { value: twinkle },    uZoom:         { value: zoom },
    ...
    return () => {
      ...
      callIfFn(programRef.current, 'remove'); callIfFn(geometryRef.current, 'remove');
      callIfFn(meshRef.current, 'remove');    callIfFn(rendererRef.current, 'destroy');
